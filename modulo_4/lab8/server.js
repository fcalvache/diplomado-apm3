...
app.set('port', process.env.PORT || 3636); // asigna el puerto 
...
app.set('views', path.join(__dirname, 'views'));  // lugar donde van a estar las vistasx
...
app.use(express.static(path.join(__dirname, 'assets'))); // sitio donde estan los css, imagenes, javascript
...
app.get('*', function(req, res){
	res.status(405).send('Method not allowed - pagina no encontrada'); // si no esta una pagina de router aparecera metho not allowed
});
